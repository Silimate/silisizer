...
uint8_t directionCode(const PortDirection *dir) {
...
class LibWriter {
public:
  explicit LibWriter(LibertyLibrary *lib) : lib_(lib) {}

  bool write(const char *path, std::string &error);
...
void LibWriter::writePortRef(const LibertyPort *port) {
...
void LibWriter::writeAxisRef(const TableAxisPtr &axis) {
  if (!axis) {
    w_.u32(kNullId);
    return;
  }
...
void LibWriter::writeTableRef(const TablePtr &table) {
  if (!table) {
    w_.u32(kNullId);
    return;
  }
...
void LibWriter::writeTableModel(const TableModel *model) {
  if (model == nullptr) {
    w_.boolean(false);
    return;
  }
...
void LibWriter::writeTableModels(const TableModels *models) {
  if (models == nullptr) {
    w_.boolean(false);
    return;
  }
...
void LibWriter::writeModel(const TimingModel *model) {
...
void LibWriter::writeFuncExpr(const FuncExpr *expr) {
  if (expr == nullptr) {
    w_.u8(0xFF);
    return;
  }
...
void LibWriter::writeAttrs(TimingArcSet *set) {
...
void LibWriter::writeArcSet(TimingArcSet *set) {
...
void LibWriter::writeRiseFallMinMax(const LibertyPort *port) {
...
void LibWriter::writeMinMaxLimit(const LibertyPort *port,
...
void LibWriter::writePortAttrs(LibertyPort *port) {
...
void LibWriter::writePort(LibertyPort *port) {
  w_.str(port->name());
  if (port->isBus()) {
...
void LibWriter::writeCell(LibertyCell *cell) {
  w_.str(cell->name());
  ...
  for (const GeneratedClock *gc : gen_clks) {
    ...
    if (edges)
      for (int edge : *edges) w_.i32(edge);
...
void LibWriter::writeUnit(const Unit *unit) {
  w_.f32(unit->scale());
  w_.str(unit->suffix());
  w_.i32(unit->digits());
}

void LibWriter::writeLibrary() {
  w_.str(lib_->name());
  ...
  // entries would need a fork accessor. They only matter under
...
bool LibWriter::write(const char *path, std::string &error) {
  writeLibrary();

  FILE *f = fopen(path, "wb");
  if (f == nullptr) {
    error = std::string("could not open ") + path + " for writing";
    return false;
  }
...
bool writeLibDb(const char *path, LibertyLibrary *library, std::string &error) {
  if (library == nullptr) {
    error = "no liberty library to write";
    return false;
  }
