...
PortDirection *directionFromCode(uint8_t code) {
  switch (code) {
...
class LibLoader {
public:
  LibLoader(DbReader &r, Network *network, Report *report, Debug *debug,
            bool infer_latches)
      : r_(r), network_(network), report_(report), debug_(debug),
        builder_(debug, report), infer_latches_(infer_latches) {}

  LibertyLibrary *read(std::string &error);
...
LibertyPort *LibLoader::readPortRef(LibertyCell *cell) {
  if (!r_.boolean()) return nullptr;
  const std::string &name = r_.str();
  return cell ? cell->findLibertyPort(name) : nullptr;
}

TableAxisPtr LibLoader::readAxisRef() {
  uint32_t id = r_.u32();
  if (id == kNullId) return nullptr;
  if (id < axes_.size()) return axes_[id];
...
TablePtr LibLoader::readTableRef() {
  uint32_t id = r_.u32();
  if (id == kNullId) return nullptr;
  if (id < tables_.size()) return tables_[id];
...
TableModel *LibLoader::readTableModel() {
  if (!r_.boolean()) return nullptr;
...
TableModels *LibLoader::readTableModels() {
  if (!r_.boolean()) return nullptr;
  return new TableModels(readTableModel());
}

TimingModel *LibLoader::readModel(LibertyCell *cell) {
...
FuncExpr *LibLoader::readFuncExpr(LibertyCell *cell) {
  uint8_t op_code = r_.u8();
  if (op_code == 0xFF) return nullptr;
...
TimingArcAttrsPtr LibLoader::readAttrs(LibertyCell *cell) {
  uint32_t id = r_.u32();
  ...
  for (const RiseFall *rf : RiseFall::range()) {
    ...
    if (model) attrs->setModel(rf, model);
  }
  if (id != kNullId)
...
void LibLoader::readArcSet(LibertyCell *cell) {
...
void LibLoader::readPort(LibertyCell *cell) {
  const std::string name = r_.str();
...
void LibLoader::readPortAttrs(LibertyCell *cell) {
  LibertyPort *port = cell->findLibertyPort(r_.str());

  PortDirection *dir = directionFromCode(r_.u8());
  if (port) port->setDirection(dir);

  for (const RiseFall *rf : RiseFall::range()) {
    for (const MinMax *mm : MinMax::range()) {
      bool exists = r_.boolean();
      float value = r_.f32();
      if (port && exists) port->setCapacitance(rf, mm, value);
    }
  }
  for (const MinMax *mm : MinMax::range()) {
    bool exists = r_.boolean();
    float value = r_.f32();
    if (port && exists) port->setSlewLimit(value, mm);
  }
  for (const MinMax *mm : MinMax::range()) {
    bool exists = r_.boolean();
    float value = r_.f32();
    if (port && exists) port->setCapacitanceLimit(value, mm);
  }
  for (const MinMax *mm : MinMax::range()) {
    bool exists = r_.boolean();
    float value = r_.f32();
    if (port && exists) port->setFanoutLimit(value, mm);
  }

  bool exists = r_.boolean();
  float value = r_.f32();
  if (port && exists) port->setFanoutLoad(value);

  exists = r_.boolean();
  value = r_.f32();
  if (port && exists) port->setMinPeriod(value);

  for (const RiseFall *rf : RiseFall::range()) {
    exists = r_.boolean();
    value = r_.f32();
    if (port && exists) port->setMinPulseWidth(rf, value);
  }
  ...
  if (cg_clk) cell->setHasClkGateClkPin();
  if (cg_enable) cell->setHasClkGateEnablePin();
  if (port) {
  ...
  if (r_.boolean()) {
    ...
    if (port) port->setPulseClk(trigger, sense);
  }
  ...
  if (port) {
    if (func) port->setFunction(func);
    if (tristate) port->setTristateEnable(tristate);
    if (related_power) port->setRelatedPowerPort(related_power);
    if (related_ground) port->setRelatedGroundPort(related_ground);
  }
}

////////////////////////////////////////////////////////////////

void LibLoader::readCell() {
  const std::string name = r_.str();
...
void LibLoader::readUnit(Unit *unit) {
  unit->setScale(r_.f32());
  unit->setSuffix(r_.str().c_str());
  unit->setDigits(r_.i32());
}

LibertyLibrary *LibLoader::read(std::string &error) {
  const std::string name = r_.str();
  const std::string filename = r_.str();
  lib_ = network_->makeLibertyLibrary(name, filename);
  if (lib_ == nullptr) {
    error = "could not create liberty library";
    return nullptr;
  }
  ...
  for (const RiseFall *rf : RiseFall::range()) {
    bool exists = r_.boolean();
    float value = r_.f32();
    if (exists) lib_->setDefaultIntrinsic(rf, value);
  }
  for (const RiseFall *rf : RiseFall::range()) {
    bool exists = r_.boolean();
    float value = r_.f32();
    if (exists) lib_->setDefaultBidirectPinRes(rf, value);
  }
  for (const RiseFall *rf : RiseFall::range()) {
    bool exists = r_.boolean();
    float value = r_.f32();
    if (exists) lib_->setDefaultOutputPinRes(rf, value);
  }

  bool exists = r_.boolean();
  float value = r_.f32();
  if (exists) lib_->setDefaultFanoutLoad(value);
  exists = r_.boolean();
  value = r_.f32();
  if (exists) lib_->setDefaultMaxCapacitance(value);
  exists = r_.boolean();
  value = r_.f32();
  if (exists) lib_->setDefaultMaxFanout(value);
  exists = r_.boolean();
  value = r_.f32();
  if (exists) lib_->setDefaultMaxSlew(value);
  ...
  if (r_.failed()) {
    error = "liberty database is truncated or corrupt";
    return nullptr;
  }
  return lib_;
}
...
LibertyLibrary *readLibDb(const char *path, Network *network, Report *report,
                          Debug *debug, std::string &error) {
  ...
  if (fread(&hdr, sizeof hdr, 1, f) != 1) {
    error = std::string(path) + " is not a liberty database";
    fclose(f);
    return nullptr;
  }
  if (std::memcmp(hdr.magic, kLibDbMagic, sizeof hdr.magic) != 0) {
    error = std::string(path) + " is not a liberty database";
    fclose(f);
    return nullptr;
  }
  if (hdr.version != kLibDbVersion) {
    error = std::string(path) + " was written by a different silisizer version";
    fclose(f);
    return nullptr;
  }
  if (hdr.pointer_size != sizeof(void *)) {
    error = std::string(path) + " was written on a different architecture";
    fclose(f);
    return nullptr;
  }
  ...
  fclose(f);
  if (!ok) {
    error = std::string(path) + " is truncated";
    return nullptr;
  }
  ...
  strings.reserve(string_count);
  {
    ...
    for (uint32_t i = 0; i < string_count; i++) {
      if (pos + sizeof(uint32_t) > string_bytes.size()) {
        error = std::string(path) + " has a corrupt string table";
        return nullptr;
      }
      uint32_t len;
  ...
  LibLoader loader(reader, network, report, debug, hdr.infer_latches != 0);
  return loader.read(error);
}
